...
USER root
RUN apk add --update unzip wget curl docker jq openssh rsync lynx coreutils
...
RUN mkdir $HADOOP_PREFIX/etc/input $HADOOP_PREFIX/logs && \
	cp $HADOOP_PREFIX/etc/hadoop/*.xml  $HADOOP_PREFIX/etc/input&& \
